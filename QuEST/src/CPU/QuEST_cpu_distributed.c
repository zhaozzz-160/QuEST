...
// Complex datatype (not in C Binding)
MPI_Datatype MPI_QuEST_COMPLEX;
...
QuESTEnv createQuESTEnv(void) {
    ...
    if (!initialized){
        ...
        // Define custom MPI type to send QuEST Complexes:
        MPI_Type_contiguous(2, MPI_QuEST_REAL, &MPI_QuEST_COMPLEX);
        MPI_Type_commit(&MPI_QuEST_COMPLEX);
...
qreal statevec_getRealAmp(Qureg qureg, long long int index){
    ...
    if (qureg.chunkId==chunkId){
        el = qureg.stateVec[index-chunkId*qureg.numAmpsPerChunk].real;
    }
...
qreal statevec_getImagAmp(Qureg qureg, long long int index){
    ...
    qreal el;
    if (qureg.chunkId==chunkId){
        el = qureg.stateVec[index-chunkId*qureg.numAmpsPerChunk].imag;
    }
    MPI_Bcast(&el, 1, MPI_QuEST_REAL, chunkId, MPI_COMM_WORLD);
    return el;
}
...
void copyVecIntoMatrixPairState(Qureg matr, Qureg vec) {
    ...
    memcpy(&matr.pairStateVec[myOffset], vec.stateVec, numLocalAmps * sizeof(Complex) );

    // work out how many messages needed to send vec chunks (2GB limit)
    long long int maxMsgSize = MPI_MAX_AMPS_IN_MSG / 2; // Now sending as complexes -> Twice as big
    ...
    for (int broadcaster=0; broadcaster < vec.numChunks; broadcaster++) {
        ...
        for (int i=0; i< numMsgs; i++) {
            ...
            MPI_Bcast(&matr.pairStateVec[otherOffset + i * maxMsgSize], maxMsgSize,
                      MPI_QuEST_COMPLEX, broadcaster, MPI_COMM_WORLD);
        }
    }
}
...
void exchangeStateVectors(Qureg qureg, int pairRank){
    ...
    // to be safe
    long long int maxMessageCount = MPI_MAX_AMPS_IN_MSG/2;  // Now sending as complexes -> Twice as big
    ...
    for (i=0; i<numMessages; i++){
        ...
        MPI_Sendrecv(&qureg.stateVec[offset], maxMessageCount, MPI_QuEST_COMPLEX, pairRank, TAG,
                     &qureg.pairStateVec[offset], maxMessageCount, MPI_QuEST_COMPLEX,
...
void exchangePairStateVectorHalves(Qureg qureg, int pairRank){
    ...
    for (i=0; i<numMessages; i++){
        offset = i*maxMessageCount;
        MPI_Sendrecv(&qureg.stateVec[offset+numAmpsToSend], maxMessageCount, MPI_QuEST_COMPLEX, pairRank, TAG,
                     &qureg.pairStateVec[offset], maxMessageCount, MPI_QuEST_COMPLEX,
                     pairRank, TAG, MPI_COMM_WORLD, &status);
    }
}
...
# endif
        ...
        for (thisTask=0; thisTask<numTasks; thisTask++) {
            ...
            qureg.pairStateVec[thisTask+numTasks] = qureg.stateVec[thisIndex];
...
# endif
        ...
        for (thisTask=0; thisTask<numTasks; thisTask++) {
            ...
            //      + pair[thisTask])/2
            qureg.pairStateVec[thisTask+numTasks*2] = qureg.stateVec[thisIndex];
        }
    }
}
